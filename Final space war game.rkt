...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname |Final space war game|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;;;;;
;;;;; ============================================================================
;;;;; *** Spacewar! ***
;;;;;
...
(define (key-event.v3 sw ke)
  (cond
    ...
    [(key=? "u" ke) (make-sw (sw-wedge sw) (turn-ship-clockwise (sw-needle sw)))]
    [(key=? "." ke) (make-sw (sw-wedge sw) (thrust (sw-needle sw)))]
    [(key=? "e" ke) (make-sw (sw-wedge sw) (fire-laser (sw-needle sw)))]
    [else sw]))
